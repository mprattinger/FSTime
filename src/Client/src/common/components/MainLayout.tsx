  ...
  return (
    <div className="w-full h-full absolute bg-gradient-to-r from-blue-400 to-emerald-400">
      <header className="flex justify-between items-center text-black py-3 px-8 md:px-20 bg-white drop-shadow-md">
        ...
        <Menu className="xl:hidden block cursor-pointer" size={32} onClick={() => setIsMenuOpen((p) => !p)} />

        <div className={`absolute xl:hidden top-24 left-0 w-full bg-white flex flex-col items-center transform transitition-transform ${isMenuOpen ? "opacity-100" : "opacity-0"}`}
